...
/**
 * 网络管理工具
 * 部分方法需要权限ACCESS_NETWORK_STATE/ACCESS_WIFI_STATUS等,声明即可
 */
public class NetWorkUtils {
        ...
        if (context != null) {
            ...
            if (mNetworkInfo != null) {
                ...
                if (ConnectivityManager.TYPE_MOBILE == mNetworkInfo.getType()) {
                   MyLog.INSTANCE.e( "网络连接类型为：TYPE_MOBILE");
                    //判断移动网络连接状态
                    ...
                    if (STATE_MOBILE == NetworkInfo.State.CONNECTED) {
                       MyLog.INSTANCE.e( "网络连接类型为：TYPE_MOBILE, 网络连接状态CONNECTED成功！");
                        return mNetworkInfo.isAvailable();
                    }
                }
                //判断是TYPE_WIFI网络
                if (ConnectivityManager.TYPE_WIFI == mNetworkInfo.getType()) {
                   MyLog.INSTANCE.e("网络连接类型为：TYPE_WIFI");
                    //判断WIFI网络状态
                    ...
                    if (STATE_WIFI == NetworkInfo.State.CONNECTED) {
                       MyLog.INSTANCE.e("网络连接类型为：TYPE_WIFI, 网络连接状态CONNECTED成功！");
                        return mNetworkInfo.isAvailable();
                    }
                }
            }
    ...
    public static boolean isAvailableByPing(String ip) {
        ...
        CommandResult result = execCmd(String.format("ping -c 1 %s", ip), false);
