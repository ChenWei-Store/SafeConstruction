 ...
 */
class ResultCallAdapterFactory(val onNetCallback: NetworkClient.OnNetCallback?): CallAdapter.Factory() {
    /**
     * 获取自定义callAdapter
    ...
    override fun get(
        ...
        // 如果不是 HttpResult 则不由本 CallAdapter.Factory 处理
        if (getRawType(innerType) != HttpResult::class.java) return null
        // 获取后续代理
