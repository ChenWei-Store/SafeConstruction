...
class UpdateDialogActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private val listenerAdapter: OnDownloadListenerAdapter = object : OnDownloadListenerAdapter() {
        ...
        override fun downloading(max: Int, progress: Int) {
            kotlin.runCatching {
                val apkSize = DownloadManager.getInstance()?.apkSize ?: ""
                val apkSizeByte = apkSize.toDouble() * 1024 * 1024
                if (max != -1) {
                    val curr = (progress / max.toDouble() * 100.0).toInt()
                    progressBar.progress = curr
                } else if (apkSize.isNotEmpty()) {
                    val curr = (progress / apkSizeByte * 100.0).toInt()
                    progressBar.progress = curr
                } else {
